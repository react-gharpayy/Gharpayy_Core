...
import { BarChart2, List, PieChart, MapPin, Clock, Bell, GitBranch } from 'lucide-react';
...
const TABS = [
  ...
  { label: 'Org',          href: '/org',              icon: GitBranch  },
          ...
          {TABS.map(tab => {
            ...
            return (
              <button key={tab.href} onClick={() => router.push(tab.href)}
                ...
                <div className={`p-1 rounded-xl transition ${active ? 'bg-orange-50' : ''}`}>
                  <Icon className="w-4 h-4" strokeWidth={active ? 2.2 : 1.6} />
                </div>
                <span className={`text-[8px] font-medium leading-none ${active ? 'text-orange-500' : 'text-gray-400'}`}>
