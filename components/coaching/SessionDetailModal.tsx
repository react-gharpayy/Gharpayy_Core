...
import { X, Calendar as CalendarIcon, Clock, Link as LinkIcon, Video, AlertTriangle, CheckSquare, Plus, Save, Activity, Trash2, Edit2, PlayCircle, Lightbulb, History, Users, ArrowRight, TrendingDown, Target, CheckCircle2, Sparkles, FileDown, ShieldCheck, Brain } from 'lucide-react';
...
export default function SessionDetailModal({ sessionId, isOpen, onClose, onUpdate, isEmployeeView = false }: SessionDetailModalProps) {
  ...
  const [discussionAssist, setDiscussionAssist] = useState('');

  const generateAiSummary = async () => {
    if (!context?.analytics) return;
    setGenerating(true);
    try {
      const res = await fetch('/api/coaching/ai-summary', {
        method: 'POST',
  ...
  const generateAiActionItems = async () => {
    ...
    try {
      const res = await fetch('/api/coaching/action-items', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          sessionId: session._id,
          summaryContext: aiSummary,
          metricsContext: context?.analytics,
        }),
      });
      const data = await res.json();
      if (data.ok && data.actionItems) {
        const newItems = [
          ...(session.actionItems || []), 
          ...data.actionItems.map((a: any) => ({ title: a.title, description: a.description, status: 'pending' }))
        ];
        await handleSave({ actionItems: newItems });
      } else {
        alert(data.error || 'Failed to generate action items');
      }
    } catch (e: any) {
      console.error(e);
      alert('Network error: ' + e.message);
  ...
  const generateDiscussionAssist = async () => {
    setAssistLoading(true);
    try {
      const res = await fetch('/api/coaching/discussion-assist', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ notes: `Shared Notes: ${sharedNotes}\nPrivate Notes: ${privateNotes}` })
      });
      const data = await res.json();
      if (data.ok) {
        setDiscussionAssist(data.assist);
      } else {
        alert(data.error || 'Failed to generate discussion assist');
      }
    } catch (e: any) {
      console.error(e);
      alert('Network error: ' + e.message);
                    ...
                    {!isEmployeeView && (
                      <div className="space-y-3">
                        ...
                        <div className="flex justify-end pt-2">
                          <button
                            onClick={generateDiscussionAssist}
                            disabled={assistLoading || (!sharedNotes && !privateNotes)}
                            className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all ${assistLoading ? 'bg-indigo-100 text-indigo-400' : 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100 border border-indigo-100 shadow-sm'}`}
                          >
                            {assistLoading ? (
                              <><div className="w-3 h-3 border-2 border-indigo-400 border-t-transparent rounded-full animate-spin" /> Analyzing...</>
                            ) : (
                              <><Brain className="w-3.5 h-3.5" /> AI Discussion Assist</>
                            )}
                          </button>
                        </div>
                        {discussionAssist && (
                          <div className="mt-4 p-6 bg-indigo-50/30 border border-indigo-100 rounded-[2rem] text-sm text-gray-800 prose prose-sm max-w-none">
                            <ReactMarkdown>{discussionAssist}</ReactMarkdown>
                          </div>
                        )}
                      </div>
                    )}
                     ...
                     {!isEmployeeView && (
                       <div className="flex gap-3">
                         ...
                         <button 
                           ...
                           className={`px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all ${generatingActions ? 'bg-indigo-100 text-indigo-400' : 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-xl shadow-indigo-100 active:scale-95'}`}
                         >
                           {generatingActions ? (
                             <><div className="w-3 h-3 border-2 border-indigo-400 border-t-white rounded-full animate-spin" /> Generating...</>
                           ) : (
                             <><Brain className="w-3.5 h-3.5" /> AI Generate Actions</>
                           )}
                         </button>
                       </div>
                     )}
                         ...
                         session.actionItems?.map((item: any, i: number) => (
                           <div key={item._id || i} className={`p-5 rounded-2xl border flex items-center gap-4 transition-all ${item.status === 'completed' ? 'bg-gray-50 border-gray-100 opacity-60' : 'bg-white border-gray-200 shadow-sm hover:shadow-md'}`}>
                             ...
                             <div className="flex-1">
                               ...
                               {item.description && <div className={`text-[11px] mt-1 ${item.status === 'completed' ? 'text-gray-400' : 'text-gray-600'}`}>{item.description}</div>}
                   ...
                   <div className="space-y-6">
                      <div className="p-8 bg-indigo-50/30 border border-indigo-100 rounded-[2.5rem]">
                         <div className="flex items-center justify-between mb-6">
                          ...
                          {aiSummary ? (
                            <div className="w-full h-auto min-h-[18rem] p-8 bg-white border border-indigo-100 rounded-[2rem] text-sm text-gray-800 prose prose-sm max-w-none shadow-sm overflow-auto">
                               <ReactMarkdown>{aiSummary}</ReactMarkdown>
                            </div>
                          ) : (
                            <div className="w-full h-72 p-8 bg-white border border-indigo-100 rounded-[2rem] flex items-center justify-center text-sm font-medium text-gray-400 shadow-sm">
                              Summarized insights will appear here once generated...
                            </div>
                          )}
                       </div>
                   </div>
