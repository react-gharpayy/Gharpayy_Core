...
export async function POST(req: NextRequest) {
  try {
    ...
    /**
     * ARCHITECTURE NOTE:
     * playbookRole (recruiter, coach, floor_lead, comm_shield) = OPERATIONAL role
     *   → drives KPI/sprint assignment
     *   → stored on User.playbookRole
     *
     * hierarchyRole (Manager, Team Lead, Employee) = AUTHORITY role
     *   → drives permissions, reporting, approvals
     *   → stored on User.hierarchyRoleId
     *
     * teamName (HR, Engineering, Operations) = TEAM grouping
     *   → drives Arena admin view grouping, KPI dashboards
     *   → stored on User.teamName
     *
     * These are THREE separate dimensions. A user can be:
     *   teamName: "HR", hierarchyRole: "Manager", playbookRole: "coach"
     */

    const roles = ['recruiter', 'coach', 'floor_lead', 'comm_shield'];

    // 1. Seed KPIs — keyed by playbookRole (operational function)
    const kpiSeeds = [
    ...
    // 2. Seed Sprint Plans — keyed by playbookRole
    const sprintSeeds = [
    ...
    if (demoEmp) {
      ...
      await ArenaDailyState.findOneAndUpdate(
        { userId: demoEmp._id, date: today },
        {
          $set: {
            ...
            decisions: [{ text: 'Prioritized hot leads over cold calling due to end-of-month target squeeze.', timestamp: new Date() }],
            shieldMode: true,
          },
        },
        { upsert: true }
      ...
      // Ensure demo employee has a playbookRole (operational) but NOT a hierarchy role confusion
      const updates: any = {};
      if (!demoEmp.playbookRole) updates.playbookRole = 'recruiter';
      // Set jobTitle separately from hierarchy role
      if (!(demoEmp as any).jobTitle) updates.jobTitle = 'Senior Recruiter';
      if (Object.keys(updates).length) {
        await User.findByIdAndUpdate(demoEmp._id, { $set: updates });
      }
    }

    return NextResponse.json({ ok: true, message: 'Arena demo data seeded. playbookRole=operational, hierarchyRole=authority, teamName=grouping.' });
  } catch (error: any) {
